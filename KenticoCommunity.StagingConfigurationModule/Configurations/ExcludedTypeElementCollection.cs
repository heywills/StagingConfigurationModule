...
{
    /// <summary>
    /// Represents a collection of ExcludedTypeElements in the configuration, to provide a list of object types that should be
    /// excluded from staging tasks on the source server.
    /// </summary>
    ...
    public class ExcludedTypeElementCollection : ConfigurationElementCollection, IEnumerable<ExcludedTypeElement>
    {
        ...
        {
            var count = Count;
            for (var i = 0; i < count; i++) yield return BaseGet(i) as ExcludedTypeElement;
        }
